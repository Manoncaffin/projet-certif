document.addEventListener("DOMContentLoaded", function () {
    ...
    function searchAddress() {
        ...
        const materialBio = materialBioSelect ? materialBioSelect.value : "";

        const materialGeoSelect = document.getElementById("material-geo-select");
        const materialGeo = materialGeoSelect ? materialGeoSelect.value : "";
        ...
        fetch(url)
            ...
            .then(data => {
                if (data && data.length > 0) {
                    ...
                    // console.log(typeof announces)
                    let hasMatchingAnnounce = false;

                    // Supprimer tous les marqueurs précédents de la carte
                    ...
                    // announces.forEach(announce => {
                        ...
                        if (announce.material === materialBio && announce.geographicalArea === address && announce.type === 'donner') {
                        ...
                        } else if (announce.material === materialGeo && announce.geographicalArea === address && announce.type === 'donner') {
                            const marker = L.marker([announce.latitude, announce.longitude], {
                                icon: myIcon
                            }).addTo(map);
